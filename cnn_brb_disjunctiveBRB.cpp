...
float totalWeight = 0; 
float consequentBeliefDegree[9];  
//float updatedConsequentBeliefDegree[27];
float beliefDegreeChangeLevel = 0; 
float activationWeight[3];  
float ruleWiseBeliefDegreeSum[3]; 
string line;
string cnn_mild;
string cnn_nominal;
string cnn_severe;
...
{      
    ...
    consequentBeliefDegree[4] = 1;
    consequentBeliefDegree[5] = 0; 
...
{    
    ...
    for (int c = 0; c < 3; c++){
        //for (int d = 0; d < 3; d++){
                //weight[increment] = ti1[c] * ti2[d] * ti3[e];
                matchingDegree[increment] = pow(ti1[c], relativeWeight) * pow(ti2[c], relativeWeight);
                trainedMatchingDegree[increment] = pow(ti1[c], relativeWeight) + pow(ti2[c], relativeWeight);  
                increment++;  
    }
}  
...
{ 
    ...
    for (int counter = 0; counter < 3; counter++)
    { 
        activationWeight[counter] = trainedMatchingDegree[counter]/totalWeight; 
...
void aggregateER_BrbCnn()
{
    ...
    for (int t = 0; t < 3; t++) //modified for disjunctive BRB on 6.2.2019 from loop value 9 to 3 as number of rules is now 3, NOT 9
    {   
        parse = t * 3;  
        ruleWiseBeliefDegreeSum[t] = consequentBeliefDegree[parse] + consequentBeliefDegree[parse+1] + consequentBeliefDegree[parse+2];
    }
      
    for (int rule = 0; rule < 3; rule++){ //modified for disjunctive BRB on 6.2.2019 
        part11 *= (activationWeight[rule] * consequentBeliefDegree[move1] + 1 - (activationWeight[rule] * ruleWiseBeliefDegreeSum[rule]));        
        move1 += 3;
    }   
...
int main()
{
    ruleBase(); //modified for disjunctive BRB on 6.2.2019 
    takeInput();  
    ...
    takeCnnOutput();  
    calculateMatchingDegreeBrbCnn(); //modified for disjunctive BRB on 6.2.2019  
    showMatchingDegree();
    showActivationWeight(); //modified for disjunctive BRB on 6.2.2019 
    updateBeliefDegree(); 
    aggregateER_BrbCnn(); //modified for disjunctive BRB on 6.2.2019
